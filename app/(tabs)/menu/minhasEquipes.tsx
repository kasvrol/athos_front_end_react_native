  ...
  return (
    <LayoutDefault>
      <YStack marginTop={50} gap="$4" paddingBottom="$10">
        <H3 textAlign="center" color="$color10" fontWeight={'700'}>Gerenciar Equipes</H3>
        
        {equipes.length === 0 ? (
          <Text textAlign="center" color="$color">Você ainda não criou nenhuma equipe em campeonatos.</Text>
        ) : (
          equipes.map(equipe => (
            <YStack 
              key={equipe.id} 
            ...
            >
              <XStack justifyContent="space-between" alignItems="center">
                <H3 fontSize="$6" color="$color9">{equipe.nome}</H3>
                <Shield size={20} color="$colorFocus" />
              </XStack>
              
              <Text fontSize="$3" color="$color5">Membros ({equipe.membros.length})</Text>
              <Separator />

              <YStack gap="$2">
                {equipe.membros.map(membro => (
                  <XStack key={membro.usuarioId} justifyContent="space-between" alignItems="center" backgroundColor="$background" padding="$2" borderRadius="$3">
                    ...
                    {membro.usuarioId === myId && <Text fontSize="$2" color="$colorFocus">(Capitão)</Text>}
                  </XStack>
                ))}
              </YStack>
